...
Casca HTTP fina sobre a biblioteca `emolumentos-pr` (instalada do PyPI). A regra
de negócio inteira mora na lib — esta API só recebe o pedido, chama `calcular` e
devolve o JSON. É o mesmo padrão da CLI, com roupa de web.
"""
...
@app.get("/health")
def health() -> dict:
    """Health check — usado por monitoramento/deploy."""
    return {"status": "ok"}
